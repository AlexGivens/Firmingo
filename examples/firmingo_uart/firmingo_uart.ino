// Firmingo hardware-UART bridge reference: NCM, local-only DHCP and FMGO.
#include <Arduino.h>
...
#include <ports/arduino_pico/uart.h>
...
firmingo::ArduinoPicoUart uartDriver(Serial1,uart0); // UART0 defaults to GPIO 0/1 on declared boards.
firmingo::UartBackend backend(uartDriver);
...
void setup() {
  ...
  prepareIdentity();
  const firmingo::SerialConfiguration uartConfiguration(
      115200,0,8,1,firmingo::SerialParity::none);
  if (backend.configure(uartConfiguration) != firmingo::BackendResult::ok)
    startupFault();
