...
report = {
    ...
    "expected_boot_id": "37a5706bf6ff3abb",
    "started_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
    "status": "failed",
    "purpose": "one unretried 64 KiB v4 application-stream transfer overlapping fresh IPv4 Internet access",
}
...
try:
        ...
        with ThreadPoolExecutor(max_workers=1) as pool:
            ...
            internet = subprocess.run(
                [
                    ...
                    "--write-out",
                    "%{http_code} %{remote_ip}",
                    f"https://example.com/?firmingo_application_v4={time.time_ns()}",
                ],
                capture_output=True,
                text=True,
                timeout=12,
    ...
    Path("build/application-v4-overlap.json").write_text(json.dumps(report, indent=2) + "\n")
